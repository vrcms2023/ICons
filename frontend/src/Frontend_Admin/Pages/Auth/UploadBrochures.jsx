...
import { getAdvertisementFormDynamicFields, getBrochuresFormDynamicFields, imageDimensionsJson } from "../../../util/dynamicFormFields";
...
const UploadBrochures = () => {
  ...
  const handleBrochuresDelete = (brochures) => {
    ...
    const deleteMenuItemByID = async () => {
      const response = await axiosServiceApi.delete(`/contactus/updateBrochures/${brochures.id}/`);
      ...
      customUI: ({ onClose }) => {
        return (
          <DeleteDialog
            ...
            message={
              <>
                Confirm deletion of <span>{title}</span> Brochures?
              </>
            }
          />
        );
      ...
      <div className="container-fluid pt-5 contactsList">
        <div className="row px-2 px-lg-5">
          <div className="col-sm-12 col-md-11">
            <Title title={"Add Brochures"} cssClass="fs-4 pageTitle" />
          </div>
          <div className="col-6 col-sm-3 col-md-1  justify-content-center addAdvertisement">
            <Button type="" cssClass="btn btn-outline" label="Add" handlerChange={() => addNewBrochures("brochures", true)} />
          </div>
        </div>
                ...
                {brochuresList?.map((brochures) => (
                  <tr key={brochures.id}>
                    ...
                    <td className="align-middle text-left">
                      <a href="#!" onClick={() => downloadPDF(`${baseURL}${brochures.path}`)} className="mx-1 text-dark">
                    ...
                    <td className="align-middle text-end">
                      <Link to="" onClick={() => editHandler("brochures", true, brochures)} className="p-2">
                        <i className="fa fa-pencil text-warning cursor-pointer fs-5" aria-hidden="true"></i>
                      </Link>

                      <Link to="" className=" ms-4" onClick={() => handleBrochuresDelete(brochures)}>
            ...
            {componentEdit.brochures && (
              <div className={`adminEditTestmonial selected `}>
                <SingleImageUlploadWithForm
                  ...
                  showExtraFormFields={getBrochuresFormDynamicFields(editBrochures)}
